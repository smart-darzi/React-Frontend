...
const WorkerHistory = () => {
  ...
  if (loading) return (
    <div className="h-[60vh] flex flex-col items-center justify-center space-y-4">
      ...
      <p className="text-slate-500 font-bold animate-pulse uppercase tracking-tighter">{t('Loading History...', 'تاریخ لوڈ ہو رہی ہے...')}</p>
    </div>
  );
        ...
        <div className="min-w-0">
          ...
          <p className="text-slate-500 mt-2 font-medium text-sm sm:text-lg ml-[52px] sm:ml-16">
            {t(
              ...
              `${approvedWorkers.length} ورکرز میں ${combinedHistory.length} مکمل شدہ مراحل`
            )}
          </p>
        </div>
      </motion.header>
      ...
      <div className="glass-card rounded-xl overflow-hidden min-w-0">
        <div className="p-8 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-emerald-50 border-b border-emerald-100">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <option value="all">{t('All Workers', 'تمام ورکرز')} ({combinedHistory.length})</option>
          ...
          <div className="p-16 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-400">
              {selectedWorkerId === 'all'
                ? t('No one has completed any work yet', 'ابھی تک کسی کا بھی کام مکمل نہیں ہوا')
                : t("This worker doesn't have any completed history yet", 'اس ورکر کی ابھی تک کوئی مکمل تاریخ نہیں ہے')}
            </h3>
            <p className="text-slate-400 text-sm font-medium mt-1.5">{t('It will show up here once a stage is completed.', 'جب کوئی مرحلہ مکمل ہوگا، وہ یہاں دکھے گا۔')}</p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[640px] text-sm border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-500">
                  ...
                  <th className="text-left font-black uppercase text-[10px] tracking-widest px-4 py-3">{t('Worker', 'ورکر')}</th>
                    ...
                    {g.entries.map((entry, ei) => (
                      ...
                      >
                        {ei === 0 && (
                          ...
                          >
                            <p className="font-black text-slate-800 uppercase whitespace-nowrap">{td(g.order.orderType)}</p>
                        ...
                        <td className="px-4 py-3">
                          <span className="inline-block px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-emerald-100 text-emerald-700 whitespace-nowrap">
                            {t(entry.stage, STAGE_URDU_LABELS[entry.stage] || entry.stage)}
                          </span>
                        </td>
