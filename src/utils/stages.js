...
// ✅ Old orders (created before the Order History timeline switched to
// storing bilingual "<Urdu> / <English>" description lines — see
// backend/controllers/order.controller.js's bilingualLog) have their
// history saved as a single plain Roman-Urdu/English sentence, e.g.
// "Haider ne kaam shuru kar diya". Those lines have no " / " separator for
// tdLog() to split, so they'd otherwise show up in Roman letters forever,
// in both languages. This recognizes the fixed set of old templates the
// app used to generate and rewrites them into the same bilingual
// "<Urdu> / <English>" form new lines use, so tdLog() can pick the right
// half. Free-typed content the templates wrap (a guidance note, a block
// reason) is preserved exactly as the person originally typed it — only
// the app's own surrounding wording is translated. Lines that don't match
// any known old template (including ones already bilingual) pass through
// unchanged.
const stageUrFor = (stage) => STAGE_URDU_LABELS[stage] || stage;
const LEGACY_HISTORY_PATTERNS = [
  {
    re: /^(.+?) \(Master Tailor\) ne (.+?) mukammal ki, ab (.+?) shuru$/,
    build: ([, who, from, to]) =>
      [`${who} (ماسٹر ٹیلر) نے ${stageUrFor(from)} مکمل کی، اب ${stageUrFor(to)} شروع`,
       `${who} (Master Tailor) finished ${from}, moving on to ${to}`],
  },
  {
    re: /^(.+?) ne kaam mukammal kar liya, self-review mein hai$/,
    build: ([, who]) =>
      [`${who} نے کام مکمل کر لیا، خود جائزہ لے رہا ہے`, `${who} marked complete, self-reviewing`],
  },
  {
    re: /^(.+?) ne kaam Admin ko bhej diya, manzoori ka intezar$/,
    build: ([, who]) =>
      [`${who} نے کام ایڈمن کو بھیج دیا، منظوری کا انتظار`, `${who} sent work to Admin, awaiting approval`],
  },
  {
    re: /^(.+?) ne kaam shuru kar diya$/,
    build: ([, who]) => [`${who} نے کام شروع کر دیا`, `${who} started work`],
  },
  {
    re: /^(.+?) ne rukawat report ki(?:: "([\s\S]*)")?$/,
    build: ([, who, reason]) =>
      [`${who} نے رکاوٹ کی اطلاع دی${reason ? `: "${reason}"` : ''}`,
       `${who} reported a block${reason ? `: "${reason}"` : ''}`],
  },
  {
    re: /^Admin ne rehnumai bheji: "([\s\S]*)"$/,
    build: ([, msg]) => [`ایڈمن نے رہنمائی بھیجی: "${msg}"`, `Admin sent guidance: "${msg}"`],
  },
  {
    re: /^dobara (.+?) ko (.+?) ke liye assign kiya gaya$/,
    build: ([, who, stage]) =>
      [`دوبارہ ${who} کو ${stageUrFor(stage)} کے لیے تفویض کیا گیا`, `Reassigned to ${who} for ${stage}`],
  },
  {
    re: /^(.+?) ko (.+?) ke liye assign kiya gaya$/,
    build: ([, who, stage]) =>
      [`${who} کو ${stageUrFor(stage)} کے لیے تفویض کیا گیا`, `${who} assigned to ${stage}`],
  },
  {
    re: /^Admin ne kaam manzoor kar liya$/,
    build: () => ['ایڈمن نے کام منظور کر لیا', 'Approved by Admin'],
  },
  {
    re: /^Order customer ko pohncha diya gaya$/,
    build: () => ['آرڈر کسٹمر کو پہنچا دیا گیا', 'Delivered to customer'],
  },
  {
    re: /^Order create hua$/,
    build: () => ['آرڈر بن گیا', 'Order created'],
  },
];
export const translateLegacyHistoryLine = (description) => {
  if (typeof description !== 'string') return description;
  for (const { re, build } of LEGACY_HISTORY_PATTERNS) {
    const match = description.match(re);
    if (match) {
      const [ur, en] = build(match);
      return `${ur} / ${en}`;
    }
  }
  return description;
