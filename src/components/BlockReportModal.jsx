    ...
    >
      ...
      >
        ...
        <h3 className="text-2xl font-black text-slate-800 tracking-tight mb-2">
          Rukawat Report Karein / Report a Block
        </h3>
        <p className="text-slate-500 font-medium mb-6">
          {order?.orderType} — {order?.workStage} mein kya masla aa raha hai?
        </p>

        <textarea
          dir="ltr"
          className="input-field w-full min-h-[90px] resize-none mb-6"
          placeholder="Wajah likhein (optional)... / Describe the issue (optional)..."
          value={reason}
        ...
        <p className="text-sm font-black text-slate-600 uppercase tracking-widest mb-3">
          Kya aap chahte hain Admin aapko guidelines bheje?
        </p>
        ...
        {wantsGuidance === true && (
          <p className="text-xs text-slate-400 -mt-4 mb-6">
            Admin ki guidelines aane tak aap kaam resume ya complete nahi kar sakenge.
          </p>
        )}

        {error && (
          <div className="flex items-center gap-3 bg-red-50 border border-red-100 text-red-600 px-4 py-3 rounded-2xl text-sm font-bold mb-6">
        ...
        <div className="flex gap-4">
          <button
            ...
            className="flex-1 py-4 rounded-2xl bg-red-500 hover:bg-red-600 text-white font-black shadow-xl shadow-red-100 transition-all disabled:opacity-60 flex items-center justify-center gap-2"
          >
            {submitting ? <Loader2 className="animate-spin" size={18} /> : 'Report Bhejein / Send Report'}
          </button>
          <button
            onClick={onCancel}
            disabled={submitting}
