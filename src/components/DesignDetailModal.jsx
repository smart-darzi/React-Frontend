...
const DesignDetailModal = ({ design, relatedDesigns = [], onSelectRelated, onClose }) => {
    ...
    >
      ...
      >
        <div className="relative">
          {images[activeImage] ? (
            <img
              ...
              alt={design.name}
              className="w-full h-56 md:h-64 object-contain bg-slate-50 rounded-t-[2rem]"
            />
          ) : (
            <div className="w-full h-56 md:h-64 bg-slate-100 rounded-t-[2rem]" />
          )}
          <button
            onClick={onClose}
            className="absolute top-3 right-3 p-2 bg-white/90 hover:bg-white rounded-lg shadow-lg text-slate-600 transition-all"
          >
            <X size={16} />
          </button>
          {design.isFeatured && (
            <span className="absolute top-3 left-3 flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black bg-amber-400 text-white">
          ...
          {images.length > 1 && (
            <>
              <button
                ...
                className="absolute left-3 top-1/2 -translate-y-1/2 p-2 bg-white/90 hover:bg-white rounded-lg shadow-lg text-slate-600 transition-all disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-white/90"
              >
                <ChevronLeft size={16} />
              </button>
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-white/90 hover:bg-white rounded-lg shadow-lg text-slate-600 transition-all disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-white/90"
              >
                <ChevronRight size={16} />
              </button>
              <span className="absolute bottom-3 right-3 px-2.5 py-1 rounded-full text-[10px] font-black bg-black/50 text-white backdrop-blur-sm">
          ...
          <div className="flex gap-1.5 px-6 pt-4 overflow-x-auto">
            {images.map((img, i) => (
              <button
                ...
                className={`flex-shrink-0 w-12 h-12 rounded-lg overflow-hidden border-2 transition-all ${i === activeImage ? 'border-primary' : 'border-transparent opacity-70 hover:opacity-100'}`}
              >
                <img src={img.url} className="w-full h-full object-cover" alt={`${design.name} ${i + 1}`} />
              </button>
            ))}
          </div>
        )}
        ...
        <div className="p-6 space-y-3">
          ...
          {design.description && <p className="text-slate-500 text-sm font-medium">{design.description}</p>}
              ...
              <div className="grid grid-cols-3 gap-2">
                {relatedDesigns.map(rd => (
                  <button
                    ...
                    className="rounded-lg overflow-hidden border-2 border-slate-100 hover:border-primary/40 transition-all text-left"
                  >
                    <img src={rd.images?.[0]?.url} alt={rd.name} className="w-full h-16 object-cover" />
